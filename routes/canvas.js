...
router.post('/', function(req, res, next) {
  ...
  pool.getConnection(function(err,conn){
    ...
    var sql = "SELECT * FROM canvasMeetingList";
    conn.query(sql,function(err,result){
      if(err) console.error("query connect error : " + err);
      console.log(result);
      res.render('canvaslist',{list : result});
      conn.release();
    })
  })
});
...
router.get('/:idx', function(req, res, next) {
  ...
  pool.getConnection(function(err,conn){
    if(err) console.error("pool connect error : "+err);
    var sql = "SELECT * FROM canvasMeetingList WHERE idx="+idx;
    conn.query(sql,function(err,roomInfo){
      ...
      conn.query(sql,function(err,result){
        if(err) console.error("query 2 connect error : "+err);
          res.render('canvasroom',{roomInfo : roomInfo[0], nick: userNick, email: result[0]});
          conn.release();
      });
    });
  });
...
router.get('/make/:idx',function(req,res,next){
  var idx = req.params.idx;
  res.render('mca',{idx:idx,builder:userNick});
});
router.post('/make',function(req,res,next){
  ...
  if(room_pw!=''){
    isLock=1;
    sql = "UPDATE canvasMeetingList SET name='"+room_name+"', img='"+room_image+"',password='"+room_pw+"',builder='"+room_builder+"',isLock="+isLock+" WHERE idx="+idx
  }else{
    sql = "UPDATE canvasMeetingList SET name='"+room_name+"', img='"+room_image+"',builder='"+room_builder+"',isLock="+isLock+" WHERE idx="+idx
  }
  pool.getConnection(function(err,conn){
    if(err) console.error("pool connect error : "+err);
    conn.query(sql,function(err,result){
